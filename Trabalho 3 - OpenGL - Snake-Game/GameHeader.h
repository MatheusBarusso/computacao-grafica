...
int pos_x[NUM_OBSTACLES];
int pos_y[NUM_OBSTACLES];
...
bool_t searchObstacle( int x , int y)
{
	for (int i = 0; i < NUM_OBSTACLES; i++) {
		if(checkEqualNode(pos_x[i], pos_y[i] , x , y  ))
			return TRUE;
		}
	return FALSE;
}
...
{
	for (int i = 0; i < NUM_OBSTACLES; i++) {
		glRecti( pos_x[i]-15 ,  pos_y[i]-15 , pos_x[i]+15 , pos_y[i]+15 );
	}
}
