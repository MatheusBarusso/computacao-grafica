...
# Translação -> T(x,y) = (x + tx, y + ty)

def plot_aa(surface, x, y, color, brightness):
    if 0 <= int(x) < surface.get_width() and 0 <= int(y) < surface.get_height():
        c = tuple(int(v * max(0.0, min(1.0, brightness))) for v in color)
        surface.set_at((int(x), int(y)), c)

def ipart(x): return int(x)
def roundi(x): return int(x + 0.5)
def fpart(x): return x - int(x)
def rfpart(x): return 1 - fpart(x)

def draw_line_xiaolin_wu(surface, x0, y0, x1, y1, color=(255,255,255)):
    ...
    if steep:
        plot_aa(surface, ypxl1, xpxl1, color, rfpart(yend) * xgap)
        plot_aa(surface, ypxl1 + 1, xpxl1, color, fpart(yend) * xgap)
    else:
        plot_aa(surface, xpxl1, ypxl1, color, rfpart(yend) * xgap)
        plot_aa(surface, xpxl1, ypxl1 + 1, color, fpart(yend) * xgap)
    ...
    if steep:
        plot_aa(surface, ypxl2, xpxl2, color, rfpart(yend) * xgap)
        plot_aa(surface, ypxl2 + 1, xpxl2, color, fpart(yend) * xgap)
    else:
        plot_aa(surface, xpxl2, ypxl2, color, rfpart(yend) * xgap)
        plot_aa(surface, xpxl2, ypxl2 + 1, color, fpart(yend) * xgap)

    if steep:
        for x in range(xpxl1 + 1, xpxl2):
            plot_aa(surface, ipart(intery), x, color, rfpart(intery))
            plot_aa(surface, ipart(intery) + 1, x, color, fpart(intery))
            intery += gradient
    else:
        for x in range(xpxl1 + 1, xpxl2):
            plot_aa(surface, x, ipart(intery), color, rfpart(intery))
            plot_aa(surface, x, ipart(intery) + 1, color, fpart(intery))
            intery += gradient


def translate_mesh(mesh, tx, ty):
    for v in mesh.vertices.values():
        v.x += tx
        v.y += ty


# Escala -> S(x,y) = (x*sx, y*sy)
def scale_mesh(mesh, sx, sy, center=None):
    mesh.scale_x *= sx
    mesh.scale_y *= sy
    ...
    for v in mesh.vertices.values():
        v.x = cx + (v.original_x - cx) * mesh.scale_x
        v.y = cy + (v.original_y - cy) * mesh.scale_y


# Rotação -> R(x,y)
def rotate_mesh(mesh, angle_deg, center=None):
    if center is None:
        center = get_mesh_center(mesh)
...
#Cisalhamento -> X' = x + shx*y  // Y' = y + shy*x
def shear_mesh(mesh, shx=0.0, shy=0.0):
    for v in mesh.vertices.values():
        x_old = v.x
        y_old = v.y
...
#Reflexão -> Eixo x, y ou xy
def reflect_mesh(mesh, axis='x', center=None):
    if center is None:
        center = get_mesh_center(mesh)
...
#Centro da malha p outras funcoes
...
def render_mesh(screen, mesh, width, height, font, line_algorithm):
    if not mesh.vertices: return
    
    # normalizar coordenadas
    min_x = mesh.view_min_x
    max_x = mesh.view_max_x
    min_y = mesh.view_min_y
    max_y = mesh.view_max_y
    ...
    for he in mesh.half_edges:
        ...
        if edge_id not in drawn_edges:
            ...
            if line_algorithm == "wu":
                draw_line_xiaolin_wu(screen, x0, y0, x1, y1, color=(150, 150, 150))
            else:
...
def query_menu(mesh):
    while True:
        ...
        print("="*40)
        print("Operações de busca de estrutura:")
        ...
        print("\nOperações de busca de transformação:")
        print("9. Translação")
        print("10. Escala")
        print("11. Rotação")
        print("12. Cisalhamento")
        print("13. Reflexão")
        print("\n0. Sair")
        ...
        # TRANSFORMAÇÕES
            ...
            translate_mesh(mesh, tx, ty)
            print("Translação aplicada.")


        elif op == 10:
            sx = float(input("Escala em X: "))
            sy = float(input("Escala em Y: "))

            scale_mesh(mesh, sx, sy)
            ...
            rotate_mesh(mesh, angle)
            print("Rotação aplicada.")


        elif op == 12:
            shx = float(input("Cisalhamento em X: "))
            shy = float(input("Cisalhamento em Y: "))

            shear_mesh(mesh, shx, shy)
            ...
            print("xy -> reflexão na origem")

            axis = input("Escolha o eixo: ").lower()

            reflect_mesh(mesh, axis)
...
def main():
    if len(sys.argv) < 3:
        print("Uso:")
        print("python render_halfedge.py <arquivo.obj> bresenham")
        print("python render_halfedge.py <arquivo.obj> wu")
        sys.exit(1)

    obj_path = sys.argv[1]
    algorithm = sys.argv[2].lower()

    if algorithm not in ["bresenham", "wu"]:
        print("Algoritmo inválido. Use 'bresenham' ou 'wu'.")
        sys.exit(1)
    print("Carregando malha...")
    mesh = load_obj_to_halfedge(obj_path)
    mesh.scale_x = 1.0
    mesh.scale_y = 1.0
    mesh.view_min_x = min(v.x for v in mesh.vertices.values())
    mesh.view_max_x = max(v.x for v in mesh.vertices.values())
    mesh.view_min_y = min(v.y for v in mesh.vertices.values())
    mesh.view_max_y = max(v.y for v in mesh.vertices.values())
    ...
    while running and cli_thread.is_alive():
        ...
        render_mesh(screen, mesh, width, height, font, algorithm)
